...
/*
 * Utilities class for Zulu eDM Connector
 * @since       1.0
 */
...
class Ze_Connector_Utility {
   ...
   public static function instance() {
      ...
      if ( is_null( $instance ) ) {
         $instance = new Ze_Connector_Utility();
      }
      return $instance;
   }
   ...
   public function admin_notice( $data = array() ) {
      ...
      switch ( $message_type ) {
         ...
         case 'update-nag':
            ...
            break;
         default:
            $message = __( 'There\'s something wrong with your code...', 'zeconnector' );
            $admin_notice = "<div id=\"message\" class=\"error\">\n";
            break;
      }

      $admin_notice .= "    <p>" . __( $message, 'zeconnector' ) . "</p>\n";
      $admin_notice .= "</div>\n";
      return $admin_notice;
   }
    ...
    * @since 1.0
    */
    public static function ze_debug_log($error){
		try{	
			if( ! is_dir( ZE_CONNECTOR_PATH.'logs' ) ){
				mkdir( ZE_CONNECTOR_PATH . 'logs', 0755, true );
			}
		} catch (Exception $e) {

		}
		try{
			$log = fopen( ZE_CONNECTOR_PATH . "logs/log.txt", 'a');
			if ( is_array( $error ) ) {
